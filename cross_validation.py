...
def readSplits(f, n_folds, id_cast=int):
    splits = []
...
def subsampleDevByClass(train_by_class, dev_size_by_class):
    dev_by_class = {}
    for (_class, subset) in train_by_class.items():
        dev_by_class[_class] = subset[:dev_size_by_class[_class]]
        train_by_class[_class] = subset[dev_size_by_class[_class]:]
    return train_by_class, dev_by_class
